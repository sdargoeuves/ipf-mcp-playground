...
# from . import ipf # probably not needed, as the IPFClient is used via the SDK
...
class ListFilesInVaultToolHandler(ToolHandler):
    ...
    def run_tool(self, args: dict) -> Sequence[TextContent | ImageContent | EmbeddedResource]:
        api = IPFClient(base_url=ipf_url, auth=ipf_token, verify=ipf_verify)
    ...
    def run_tool(self, args: dict) -> Sequence[TextContent | ImageContent | EmbeddedResource]:
        ...
        for result in results:
            ...
            formatted_results.append(
                {"filename": result.get("filename", ""), "score": result.get("score", 0), "matches": formatted_matches}
            )
